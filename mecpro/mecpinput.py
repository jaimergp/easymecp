...
def _parsehessian(section, hessian=None):
    ...
    hessian = matlib.empty((size, size), dtype=float64)
    r, c = 0, 0
    for line in section:
...
_schema = {
    ...
    'general': {
        ...
        'read_later': switch,  # setting that removes guess keyword if the check file is empty
        # Aliases
        'method': 'basis',
...
def _parsearguments():
    ...
    parser.add_argument("-f", "--filename", dest="filename", type=lambda x: is_valid_file(parser, x),
                        help='name of .mecp file', required=True)
